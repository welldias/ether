...
/*
Simplex Noise in 2D, 3D and 4D. Based on the libnoise-dotnet code of inspired by "Simplex noise demystified" paper. Link:
...
static void   _ether_simple_noise_randomize(int seed);
static double _ether_simple_noise_dot2(int g[], double x, double y);
static double _ether_simple_noise_dot3(int g[], double x, double y, double z);
static double _ether_simple_noise_dot4(int g[], double x, double y, double z, double t);
static int    _ether_simple_noise_fast_floor(double x);
static void   _ether_simple_noise_fast_unpack_little_uint32(int value, char* buffer, int sizeBuffer);
...
void ether_simple_noise_init(int seed) 
{
    ...
    randomTable = calloc(((size_t)RANDOM_SIZE * 2), sizeof(int));
    if (randomTable == NULL)
        return;
