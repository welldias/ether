...
EtherScalar  ether_math_scalar_abs (EtherScalar alpha)
{
  return (EtherScalar)fabs(alpha);
}

EtherScalar ether_math_scalar_round(EtherScalar alpha)
{
  return ETHER_FLOAT_TO_SCALAR(floor(ETHER_SCALAR_TO_FLOAT(alpha) + 0.5));
}

EtherFactor ether_math_scalar_divide(EtherScalar alpha, EtherScalar beta)
{
  return (EtherFactor)(((float)(alpha)) / ((float)(beta)));
}

EtherScalar ether_math_scalar_mult_div(EtherScalar alpha, EtherScalar beta, EtherScalar gamma)
{
	return (EtherScalar)((((float)(alpha)) * ((float)(beta))) / ((float)(gamma)));
}

EtherScalar ether_math_factor_multiply(EtherFactor alpha, EtherScalar beta)
{
	return (EtherScalar)(((float)(alpha)) * ((float)(beta)));
}

EtherFactor ether_math_sine(EtherAngle angle)
{
	return ETHER_FLOAT_TO_FACTOR(sin((PI * ETHER_ANGLE_TO_FLOAT(angle)) / 180.0));
}

EtherFactor ether_math_cosine(EtherAngle angle)
{
