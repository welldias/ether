...
void
_ether_transform(EtherVector result, EtherMatrix m, EtherVector v)
{
...
EtherScalar
_ether_transform_x(EtherMatrix m, EtherVector v)
{
    return ether_vector_dotproduct(m[X], v) + m[_ETHER_TRANS][X];
}

EtherScalar
_ether_transform_y(EtherMatrix m, EtherVector v)
{
    return ether_vector_dotproduct(m[Y], v) + m[_ETHER_TRANS][Y];
}

EtherScalar
_ether_transform_z(EtherMatrix m, EtherVector v)
{
