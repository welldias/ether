...
static char          _ether_display_description_error[100];
...
ether_display_init(EtherRaster* raster)
{
	...
	n = _ether_video_current_driver(ETHER_VIDEO_SETUP, 0, NULL);
	if (n)
		return n;
...
ether_display_quit(void)
{
	...
	(*_ether_video_current_driver)(ETHER_VIDEO_SHUTDOWN, 0, NULL);
}
...
{
	(*_ether_display_current_driver)(ETHER_DISPLAY_GET_DESCRIPTION, sizeof(_ether_display_current_desc) - 1, _ether_display_current_desc);
...
/* Video functions  */
EtherDisplayStatus ether_display_status_get()
{
	return(EtherDisplayStatus)(*_ether_video_current_driver)(ETHER_VIDEO_GET_STATUS, 0, NULL);
}
...
{
	(*_ether_video_current_driver)(ETHER_VIDEO_GET_DESCRIPTION, sizeof(_ether_video_current_desc) - 1, _ether_video_current_desc);
	return _ether_video_current_desc;
}

int
ether_video_error_get(const char** description)
{
	int codeError = (*_ether_video_current_driver)(ETHER_VIDEO_GET_ERROR, sizeof(_ether_display_description_error) - 1, _ether_display_description_error);

	if (description != NULL) {
		*description = _ether_display_description_error;
	}
