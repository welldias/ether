...
EtherWorld *
ether_world_create(void)
{
    EtherWorld *w;
    w = malloc(sizeof(EtherWorld));
    if(w)
...
EtherWorld *
ether_world_init(EtherWorld *world)
{
    if(world == NULL)
        return NULL;
    world->objects = NULL;
...
void
ether_world_add_light(EtherLight *light)
{
...
void
ether_world_remove_light(EtherLight *light)
{
    EtherLight *p;
    if(ether_world_current == NULL || light == NULL) 
        return;
...
EtherLight *
ether_world_find_light(char *name)
{
    EtherLight *p;
    for(p = ether_world_current->lights; p; p = p->next)
...
void 
ether_world_add_abject(EtherObject *obj)
{
...
void 
ether_world_remove_abject(EtherObject *obj)
{
    EtherObject *p;
    if(ether_world_current == NULL || obj == NULL)
        return;
...
EtherObject *
ether_world_find_object(char *name)
{
...
int 
ether_world_count_objects(void)
{
...
void 
ether_world_bounds_get(EtherVector v1, EtherVector v2)
{
...
void
ether_world_center_get(EtherVector v)
{
    EtherVector v1, v2;
...
EtherObject *
ether_world_object_tree_get(void)
{
...
EtherObject *
ether_world_update(void)
{
    return ether_object_update(ether_world_object_tree_get());
}

void
ether_world_current_set(EtherWorld *world)
{
    ether_world_current = world;
}

EtherWorld *
ether_world_current_get(void)
{
