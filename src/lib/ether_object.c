...
static EtherVector EtherVectorNULL = {0, 0, 0};
...
EtherObject *ether_object_init(EtherObject *obj) {
  if (obj == NULL)
    return NULL;
  ...
  obj->function = NULL;
  ether_vector_copy(obj->minbound, EtherVectorNULL);
  ether_vector_copy(obj->maxbound, EtherVectorNULL);
  obj->contents = NULL;
  obj->next = NULL;
  obj->forced_rep = NULL;
  obj->name = "No Name";
...
EtherObject *ether_object_create(EtherShape *shape) {
...
EtherObject *ether_object_copy(EtherObject *proto) {
...
void ether_object_destroy(EtherObject *obj) {
  if (obj) {
...
void ether_object_move(EtherObject *obj, EtherScalar x, EtherScalar y,
                       EtherScalar z) {
  EtherVector v;
...
void ether_object_rel_move(EtherObject *obj, EtherScalar x, EtherScalar y, EtherScalar z) {
  EtherVector v;
  if (obj == NULL)
    return;
...
void ether_object_rot_vector(EtherObject *obj, EtherAngle angle, EtherVector vector) {
  if (obj == NULL)
    return;
...
void ether_object_rot_reset(EtherObject *obj) {
  if (obj == NULL)
    return;
...
void ether_object_rotate(EtherObject *obj, EtherAngle angle, int axis, EtherCoordFrame frame, EtherObject *relative_to) {
...
void ether_object_translate(EtherObject *obj, EtherVector v, EtherCoordFrame frame, EtherObject *relative_to) {
  EtherVector our_v;
  if (obj == NULL)
    return;
...
void ether_object_look_at(EtherObject *obj, EtherVector forward, EtherVector up) {
  EtherVector x, y;
  EtherMatrix mat;
...
EtherObject *ether_object_attach(EtherObject *obj, EtherObject *newparent) {
  EtherMatrix tmp;
...
EtherObject *ether_object_detach(EtherObject *obj) {
  EtherObject *oldparent;
  if (obj == NULL)
    return NULL;
...
EtherObject *ether_object_update(EtherObject *obj) {
  if (obj == NULL)
    return NULL;
...
void ether_object_traverse(EtherObject *obj,
                           int (*function)(EtherObject *obj)) {
  EtherObject *o;
...
void ether_object_make_fixed(EtherObject *obj) {
  if (obj == NULL)
    return;
...
void ether_object_make_movable(EtherObject *obj) {
  EtherShape *shape;
...
EtherObject *ether_object_find_root(EtherObject *obj) {
  if (obj == NULL)
    return NULL;
...
EtherScalar ether_object_compute_distance(EtherObject *obj1, EtherObject *obj2) {
...
void ether_object_rot_x(EtherObject *obj, EtherAngle angle) {
  if (obj)
    ether_object_rotate(obj, angle, X, ETHER_COORD_PARENT, NULL);
}

void ether_object_rot_y(EtherObject *obj, EtherAngle angle) {
  if (obj)
    ether_object_rotate(obj, angle, Y, ETHER_COORD_PARENT, NULL);
}

void ether_object_rot_z(EtherObject *obj, EtherAngle angle) {
  if (obj)
    ether_object_rotate(obj, angle, Z, ETHER_COORD_PARENT, NULL);
}

void ether_object_vector_move(EtherObject *obj, EtherVector v) {
  if (obj)
    ether_object_move(obj, v[X], v[Y], v[Z]);
}

void ether_object_vector_rel_move(EtherObject *obj, EtherVector v) {
  if (obj)
    ether_object_translate(obj, v, ETHER_COORD_PARENT, NULL);
}

void ether_object_layer_set(EtherObject *obj, unsigned char layer) {
  if (obj)
    obj->layer = layer;
}

unsigned char ether_object_layer_get(EtherObject *obj) {
  if (obj)
    return obj->layer;
...
void ether_object_shape_set(EtherObject *obj, EtherShape *shp) {
  if (obj)
    obj->shape = shp;
}

EtherShape *ether_object_shape_get(EtherObject *obj) {
  if (obj)
    return obj->shape;
  else
    return NULL;
...
void ether_object_surfacemap_set(EtherObject *obj, EtherSurfaceMap *map) {
  if (obj)
    obj->surfmap = map;
}

EtherSurfaceMap *ether_object_surfacemap_get(EtherObject *obj) {
  if (obj)
    return obj->surfmap;
  else
    return NULL;
...
void ether_object_highlight_set(EtherObject *obj, int high) {
  if (obj)
    obj->highlight = high;
}

int ether_object_highlight_get(EtherObject *obj) {
  if (obj)
...
void ether_object_toggle_highlight(EtherObject *obj) {
  if (obj)
    obj->highlight = !(obj->highlight);
}

void ether_object_visibility_set(EtherObject *obj, int vis) {
  if (obj)
    obj->invisible = vis ? 1 : 0;
}

int ether_object_visibility_get(EtherObject *obj) {
  if (obj)
    return obj->invisible;
  return 0;
}

void ether_object_toggle_visibility(EtherObject *obj) {
  if (obj)
    obj->invisible = !(obj->invisible);
}

int ether_object_is_fixed(EtherObject *obj) {
  if (obj)
    return obj->fixed;
  return 0;
}

EtherVector *ether_object_minbounds_get(EtherObject *obj, EtherVector v) {
  if (obj)
...
EtherVector *ether_object_maxbounds_get(EtherObject *obj, EtherVector v) {
  if (obj)
...
void ether_object_rep_set(EtherObject *obj, EtherRep *r) {
  if (obj)
    obj->forced_rep = r;
}

EtherRep *ether_object_rep_get(EtherObject *obj) {
  if (obj)
...
EtherScalar ether_object_world_y_get(EtherObject *obj) {
  if (obj)
    return obj->globalmat[3][Y];
  return 0;
}

EtherScalar ether_object_world_z_get(EtherObject *obj) {
  if (obj)
    return obj->globalmat[3][Z];
  return 0;
}

EtherVector *ether_object_world_location_get(EtherObject *obj, EtherVector v) {
  if (obj)
...
EtherScalar ether_object_relative_x_get(EtherObject *obj) {
  if (obj)
    return obj->localmat[3][X];
  return 0;
}

EtherScalar ether_object_relative_y_get(EtherObject *obj) {
  if (obj)
    return obj->localmat[3][Y];
  return 0;
}

EtherScalar ether_object_relative_z_get(EtherObject *obj) {
  if (obj)
    return obj->localmat[3][Z];
  return 0;
}

EtherVector *ether_object_relative_location_get(EtherObject *obj,
                                                EtherVector v) {
  if (obj)
...
void ether_object_world_rotations_get(EtherObject *obj, EtherAngle *rx, EtherAngle *ry, EtherAngle *rz) {
  if (obj)
...
void ether_object_relative_rotations_get(EtherObject *obj, EtherAngle *rx, EtherAngle *ry, EtherAngle *rz) {
  if (obj)
...
void ether_object_name_set(EtherObject *obj, char *str) {
  if (obj)
    obj->name = str;
}

char *ether_object_name_get(EtherObject *obj) {
  if (obj)
    return obj->name;
  else
    return NULL;
...
void ether_object_application_data_set(EtherObject *obj, void *data) {
  if (obj)
    obj->applic_data = data;
}

void *ether_object_application_data_get(EtherObject *obj) {
  if (obj)
...
void ether_object_forward_vector_get(EtherObject *obj, EtherVector v) {
  if (obj)
    ether_matrix_basis_get(v, obj->globalmat, Z);
}

void ether_object_right_vector_get(EtherObject *obj, EtherVector v) {
  if (obj)
    ether_matrix_basis_get(v, obj->globalmat, X);
}

void ether_object_up_vector_get(EtherObject *obj, EtherVector v) {
  if (obj)
    ether_matrix_basis_get(v, obj->globalmat, Y);
}

EtherObject *ether_object_parent_get(EtherObject *obj) {
  if (obj)
    return obj->parent;
  else
    return NULL;
...
void ether_object_function_set(EtherObject *obj, EtherObjectFunction function) {
  if (obj)
    obj->function = function;
}

EtherObjectFunction ether_object_function_get(EtherObject *obj) {
  if (obj)
    return obj->function;
  else
    return NULL;
...
EtherObject *ether_object_load_from_plg(FILE *in) {
...
static void _ether_object_transform_bbox(EtherObject *object) {
  ...
  if (!object->rotate_box && object->shape) {
    ether_vector_add(object->minbound, object->shape->minbound,
                     object->globalmat[_ETHER_TRANS]);
    ether_vector_add(object->maxbound, object->shape->maxbound,
                     object->globalmat[_ETHER_TRANS]);
    return;
  }
  ...
  for (i = 0; i < 3; ++i) {
    for (j = 0; j < 3; ++j) {
      EtherScalar a =
          ether_math_factor_multiply(object->globalmat[i][j], (*minb)[j]);
      EtherScalar b =
          ether_math_factor_multiply(object->globalmat[i][j], (*maxb)[j]);
      if (a < b) {
        object->minbound[i] += a;
        object->maxbound[i] += b;
      } else {
...
static void _ether_object_recursive_updater(EtherObject *object, int flag) {
  while (object) {
    ...
    if ((flag || object->moved) && !ether_object_is_fixed(object)) {
      ...
      /* no parent; our localmat *is* our globalmat */
      if (object->parent == NULL) 
        ether_matrix_copy(object->globalmat, object->localmat);
      /* same if our parent is fixed */
      else if (ether_object_is_fixed( object->parent))
        ether_matrix_copy(object->globalmat, object->localmat);
      /* normal case; multiply our parent's globalmat by our localmat to get our
       * globalmat */
      else 
