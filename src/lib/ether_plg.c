...
void ether_plg_scale_set(float x, float y, float z) {
  _ether_plg_sx = x;
  _ether_plg_sy = y;
  _ether_plg_sz = z;
}

void ether_plg_offset_set(float x, float y, float z) {
  _ether_plg_tx = x;
  _ether_plg_ty = y;
  _ether_plg_tz = z;
}

EtherShape * ether_plg_read(FILE *in) {
  char buff[100];
  int i;
  ...
  while(_ether_plg_line_get(buff, sizeof(buff), in)) {
    ...
    for(i = 0; i < nf; ++i) {
      ...
      /* if we're a detail facet attach us to our parent*/
      if(detail && parent) {
        facet->farside = parent->details;
        parent->details = facet;
      }
      /* otherwise, attach us to the rep's facet list */
      else {
        facet->farside = rep->facets;
        rep->facets = facet;
      }

      /* if we're not a detail, we're a (potenial) parent */
      if(!detail)
        parent = facet; 
    }
...
int ether_plg_write(EtherShape *shape, FILE *out) {
  int n;
...
EtherObject * ether_plg_load_object(char *filename) {
  EtherObject *obj;
...
static int _ether_plg_output_vertex(EtherVector *v, EtherVector *n) {
  ...
  fprintf(_ether_plg_outfile, "%ld %ld %ld", 
...
static int _ether_plg_output_rep(EtherRep *rep) {
  fprintf(_ether_plg_outfile, "rep_%ld %d %d %d %d\n",
